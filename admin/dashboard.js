(async function () {
  ...
  function renderOverview() {
    ...
    const completedApps = _applications.filter((a) => (a.status || '').toLowerCase() === 'delivered').length;
    const pendingApps = _applications.filter((a) => ['submitted', 'under_review'].includes((a.status || '').toLowerCase())).length;
          ...
          <div>
            ...
            <div class="dv" style="display:flex; align-items:center;">
              <select class="status-select" data-status-select="${subId}" data-ref="${refSafe}">
                ${STATUS_OPTIONS.map(([s, label]) => `<option value="${s}" ${a.status === s ? 'selected' : ''}>${label}</option>`).join('')}
              </select>
  ...
  function renderPaidOrders() {
    ...
    const paidKeys = new Set(
      _applications.filter((a) => a.status === 'paid' || a.status === 'delivered').map(groupKey)
    );
    const groups = new Map();
    _applications.forEach((a) => {
      const key = groupKey(a);
  ...
  // This is the single source of truth for the status dropdown options
  // here AND the labels shown on the public tracking page — keep both
  // files in sync if this list ever changes.
  ...
  const KNOWN_STATUSES = new Set(STATUS_OPTIONS.map(([s]) => s));
